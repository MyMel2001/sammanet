...
// - Custom <snet-script> and <snet-style> tags found in Markdown are sanitized and
//   sanitized content is injected into a safe <style> block. <snet-script> content is
//   sanitized and not executed from the page content. This is a demonstration and not
...
// Robust extraction of custom blocks
function extractBlocks(rawMd) {
  const snetScripts = [];
  const snetStyles = [];

  // Collect <snet-script> blocks (exact, non-greedy)
  let md = rawMd;
  md = md.replace(/<snet-script>([\\s\\S]*?)<\/snet-script>/gi, (m, inner) => {
    // Sanitize inner content by escaping potentially dangerous characters
    const sanitized = inner
      .replace(/&/g, '&')
      .replace(/</g, '<')
      .replace(/>/g, '>');
    snetScripts.push(sanitized);
    return ''; // remove from Markdown before conversion
  });

  // Collect <snet-style> blocks
  md = md.replace(/<snet-style>([\\s\\S]*?)<\/snet-style>/gi, (m, inner) => {
    // Sanitize style content to prevent injection
    const sanitized = inner
      .replace(/&/g, '&')
      .replace(/</g, '<')
      .replace(/>/g, '>');
    snetStyles.push(sanitized);
    return '';
  });
...
function buildEncryptedPageHtml(pageName, payload, stylesHtml) {
...
function route(req, res) {
  ...
  if (url.startsWith('/page/') && method === 'GET') {
    ...
    // join sanitized CSS blocks
    const stylesHtml = snetStyles.length > 0 ? snetStyles.join('\\n') : '';

    // Encrypt content HTML
    const payload = encryptHtml(contentHtml);
    const finalHtml = buildEncryptedPageHtml(name, payload, stylesHtml);
  ...
  if (url.startsWith('/upload') && method === 'POST') {
    ...
    req.on('data', (chunk) => {
      body += chunk;
      // basic guard against runaway body
      if (body.length > 1e6) {
        req.destroy();
      }
    });
